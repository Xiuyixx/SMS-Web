...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const actions = [
      {
        text: 'SIM1',
        icon: new URL('../assets/remote/0c98dc6c6b4e370d.png', import.meta.url).href
      },
      {
        text: 'SIM2',
        icon: new URL('../assets/remote/e63c357e3a7ae6b0.png', import.meta.url).href
      }
    ];
    if (props.number) {
    ...
    const sendSms = () => {
      ...
      axios.post(SEND_SMS, data).then(response => {
        if (response.data.code === 200) {
          Toast({
            message: '短信发送成功',
            icon: new URL('../assets/remote/7c682f7e147a30ce.png', import.meta.url).href,
          })
        const data=   {
          ...
          Toast({
            message: '短信发送失败',
            icon: new URL('../assets/remote/5d2451c1941ae791.png', import.meta.url).href,
          })
        }
      })
    }
